...
<body>
    ...
    <div class="wrapper d-flex align-items-stretch">
        ...
        <div id="content" class="p-4 p-md-5 pt-5">
            <button type="button" class="btn btn-primary" onclick="window.location.assign('resarve_maintenance.php')">My reservations</button>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Workshop owner</th>
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM `maintenance` WHERE `name` LIKE '%" . $search . "%'";
  ...
  <div class="modal-dialog">
    ...
    <form method="post">
      ...
      <div class="mb-3 row">
        ...
        <div class="mb-3 row">
                                <label for="matriqule" class="col-sm-2 col-form-label">From the hour</label>
                                <div class="col-sm-10">
      ...
      <div class="mb-3 row">
                                <label for="matriqulea" class="col-sm-2 col-form-label">Explain the problem</label>
                                <div class="col-sm-10">
                                <textarea required name="problem" style="border: solid 1px;" class="form-control" id="matriqulea"></textarea>
                                </div>
                            </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="ok" value="' . $row['id'] . '" class="btn btn-primary">Save</button>
                    ...
                    if (isset($_POST['ok'])) {
                        $id = $_POST['ok'];
                        $start = $_POST['start'];
                        $sql = "INSERT INTO `resarve_maintenance`( `user`, `maintenance`, `time`, `description`,`date`) VALUES ('" . $_SESSION['user']['id'] . "','$id','$start','" . $_POST['problem'] . "','" . $_POST['date'] . "')";
                        $result = mysqli_query($con, $sql);
                        if (!$result) {
                            echo "Error: " . $sql . "<br>" . mysqli_error($con);
                        } else {
                            echo "<script>alert('Reserved successfully');</script>";
                            echo "<script>window.location.assign('resarve_maintenance.php');</script>";
                        }
                    }
