...
<body>
    ...
    <div class="wrapper d-flex align-items-stretch">
        ...
        <div id="content" class="p-4 p-md-5 pt-5">
            <button type="button" class="btn btn-primary" onclick="window.location.assign('resarve_park.php')">My reservations</button>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM `parking` WHERE `name` LIKE '%" . $search . "%'";
...
<div class="modal fade" id="exampleModal' . $index . '" tabindex="-1" aria-labelledby="exampleModalLabel' . $index . '" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <form method="post">
      ...
      <div class="mb-3 row">
                                <label for="matriqulea" class="col-sm-2 col-form-label">To the hour</label>
                                <div class="col-sm-10">
                                    <input required type="time" name="end" style="border: solid 1px;" class="form-control" id="matriqulea">
                                </div>
                            </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="ok" value="' . $row['id'] . '" class="btn btn-primary">Save</button>
                    ...
                    if (isset($_POST['ok'])) {
                        ...
                        $start = $_POST['start'];
                        $end = $_POST['end'];
                        $sql = "INSERT INTO `resarve_parking`( `user`, `parking`, `start`, `end`,`date`) VALUES ('" . $_SESSION['user']['id'] . "','$id','$start','$end','" . $_POST['date'] . "')";
                        $result = mysqli_query($con, $sql);
                        if (!$result) {
                            echo "Error: " . $sql . "<br>" . mysqli_error($con);
                        } else {
                            echo "<script>alert('Reserved successfully');</script>";
                            echo "<script>window.location.href='resarve_park.php';</script>";
                        }
                    }
